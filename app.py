...
from app_utils import (load_artifacts, load_defaults, default_num, gpa_to_grade20, build_comparison_df, safe_feature_importance_df)

st.set_page_config(page_title="EduRisk", page_icon="📚", layout="wide")
...
def main():
    ...
    avg_evaluations = st.sidebar.slider("Avg assessments attempted", min_value=int(ev_min), max_value=int(ev_max), value=int(ev_val), step=1)

    ap_min, ap_max, ap_val = default_num(df_defaults, "avg_approved", 0, 30, 5)
    avg_approved = st.sidebar.slider("Avg modules passed", min_value=int(ap_min), max_value=int(ap_max), value=int(ap_val), step=1)

    st.sidebar.markdown("### GPA → Grade (0–20)")
    gpa1 = st.sidebar.number_input("Semester 1 GPA", min_value=0.0, max_value=4.0, value=3.0, step=0.01, format="%.2f")

    gpa2 = st.sidebar.number_input("Semester 2 GPA", min_value=0.0, max_value=4.0, value=2.0, step=0.01, format="%.2f")
